...
class ClipLoss(torch.nn.Module):
    ...
    def forward(self, image_embeds: torch.Tensor, text_embeds: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            image_embeds: torch.Tensor, shape (batch_size, embedding_dim)
            text_embeds: torch.Tensor, shape (batch_size, embedding_dim)
...
if __name__ == '__main__':
    image_embeds = torch.randn(6, 512)
    text_embeds = torch.randn(6, 512)
    loss = ClipLoss()
